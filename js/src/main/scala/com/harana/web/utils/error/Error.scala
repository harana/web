package com.harana.web.utils.error

import com.harana.web.shared
import com.harana.web.shared.{JavaScriptCause, JavaScriptError, JavaScriptPosition, JavaScriptTrace}
...
object Error {
  ...
  def fromJS: js.Function5[String, String, Int, Int, Throwable | js.Error, Unit] = (message: String, source: String, line: Int, column: Int, err: Throwable | js.Error) => {
    err match {
      case t: Throwable => toError(message, source, line, column, Some(t)).foreach(e => Http.postRelative("/error", List(), e.asJson.noSpaces))
      case e: js.Error => toError(message, source, line, column, Some(js.JavaScriptException(e))).foreach(e => Http.postRelative("/error", List(), e.asJson.noSpaces))
    }
    ()
  }
  ...
  private def toCause(consumerOption: Option[SourceMapConsumer], throwable: Throwable): JavaScriptCause = {
    consumerOption.map { consumer =>
      ...
      shared.JavaScriptCause(throwable.getLocalizedMessage, trace, None)
    }
  }
