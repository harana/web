      ...
      TwListbox(value = props.selectedItem, onChange = props.onChange, children = (listboxValue: js.Dynamic) =>
        Fragment(
          ...
          div(className := "relative")(
            TwListboxButton(className = "relative w-full cursor-default rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-teal-600 sm:text-sm sm:leading-6")(
                ...
                props.items.map { item =>
                  TwListboxOption(
                    ...
                    className = (value: js.Dynamic) => {
                      ...
                      s"${if (active) "bg-teal-600 text-white" else "text-gray-900"} relative cursor-default select-none py-2 pl-3 pr-9"
                    },
                    value = item,
                    children = (optionValue: js.Dynamic) => {
                      ...
                      Fragment(
                        ...
                        when(selected)(
                          span(className := s"${if (active) "text-white" else "text-teal-600"} absolute inset-y-0 right-0 flex items-center pr-4")(
                            CheckIcon("h-5 w-5")
                          )
                        )
                      )
