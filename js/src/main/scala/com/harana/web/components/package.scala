...
import com.harana.web.components.elements.{Dialog, DialogStyle}
import com.harana.web.handlers.navigation.NavigationStore.OpenRoute
...
package object components {
  ...
  @inline def when[A >: StatelessComponent](condition: Option[A]): ReactElement =
    if (condition == null || condition.isEmpty)
      null.asInstanceOf[ReactElement]
    else
      condition.asInstanceOf[StatelessComponent].render()
  ...
  @inline def openLink(circuit: Circuit[_], linkType: LinkType) =
    linkType match {
  ...
  object LinkType {
    ...
    case class Menu(menu: com.harana.web.external.shoelace.Menu.Props) extends LinkType
