...
import com.harana.web.external.shoelace.{Tab, TabGroup, TabPanel, Button => ShoelaceButton, Dialog => ShoelaceDialog}
	...
	private def layoutParameterGroup(group: ParameterGroup,
		...
		info.layout.map(l => l(group)) match {
			case Some(ParameterGroupLayout.Grid(columns)) =>
				List(com.harana.web.components.structure.Grid(group.parameters.map(p => parameterItem(p, s"${info.i18nPrefix}.tab.${group.name}", onChange, p == focusParameter)), columns))
