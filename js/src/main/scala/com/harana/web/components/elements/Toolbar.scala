				...
				li(key := item.id)(
					item match {
						case HeadingItem.Button(button, link, _) =>
							Link(props.circuit, link)(List(button))
						...
						case HeadingItem.IconButton(icon, tooltip, link, className, enabled, _) =>
							Button(
								...
								onClick = Some(_ => openLink(props.circuit, link)),
							...
							ButtonGroup(label = Some("button"))(
								icons.map { icon =>
									Button(
										...
										onClick = Some(_ => openLink(props.circuit, icon.link)),
