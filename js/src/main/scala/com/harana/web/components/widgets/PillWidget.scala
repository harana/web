...
import com.harana.web.external.shoelace.{Button, Dropdown, Menu}
...
@react class PillWidget extends StatelessComponent {
	...
	def render() =
		div(className := cssSet("pill panel" -> true), style := ColorUtils.css(props.background))(
			...
			div(className := "panel-body", onClick := (event => if (props.link.nonEmpty) openLink(props.circuit, props.link.get)))(
				h3(className := "heading-text")(span(props.title)),
				props.subtitle
			),
			div(className := "container-fluid", onClick := (event => if (props.link.nonEmpty) openLink(props.circuit, props.link.get)))(
