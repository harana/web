...
import com.harana.web.components.{cssSet, emptyElement, when}
import com.harana.web.external.shoelace.{Icon, IconButton, Menu}
...
@react class Sidebar extends StatelessComponent {
	...
	def renderSection(section: SidebarSection): ReactElement = {
		if (section.visible.isEmpty || section.visible.get())
