...
object GenStateLens {
  def generate[M: c.WeakTypeTag, A: c.WeakTypeTag, B: c.WeakTypeTag](c: blackbox.Context)(field: c.Expr[A => B], subscribe: c.Expr[Boolean]): c.Expr[B] = {
    ...
    c.Expr[B](q"""if ($subscribe) com.harana.web.utils.DiodeUtils.use(Circuit.diodeContext, $res)._1 else $res.value""")
  }
}