package com.harana.web.pages.grid
...
import com.harana.sdk.shared.models.common.{Id, User}
import com.harana.sdk.shared.models.flow.parameters.Parameter
import com.harana.sdk.shared.utils.HMap
import com.harana.web.pages.{FilterItem, ViewMode}
...
abstract case class GridHandler[AppState, Entity <: Id, S](entityType: EntityType, state: ModelRW[AppState, GridState[Entity, S]])
                                                          (implicit decoder: Decoder[Entity], encoder: Encoder[Entity]) extends ActionHandler(state) {
  ...
  def toEntity(userId: User.UserId, editedItem: Option[Entity], subType: Option[EntitySubType], values: HMap[Parameter.Values]): Entity
  ...
  def gridHandle: Option[PartialFunction[Any, ActionResult[AppState]]] = None
  override def handle = if (gridHandle.nonEmpty) gridHandle.get.orElse(commonHandle) else commonHandle
  def commonHandle: PartialFunction[Any, ActionResult[AppState]] = {
    ...
    case SaveNewItem(e, userId, parameterValues) =>
      if (e.equals(entityType)) {
        val entityTypeItem = toEntity(userId, None, value.entitySubType, parameterValues)
        effectOnly(
    ...
    case SaveExistingItem(e, userId, entityTypeId, parameterValues) =>
      if (e.equals(entityType))
        effectOnly(
          Effect.action(Block(e)) >>
            Effect(
              for {
                ...
                entity        =  toEntity(userId, existingItem, value.entitySubType, parameterValues)
